...
class Animal(Organismo):
    ...
    def reproducirse(self, otros_animales):
        for animal in otros_animales:
            # Verificar si el animal es de la misma especie, género y cercano
            if (
                animal.nombre != self.nombre
                and animal.especie == self.especie
                and animal.sexo != self.sexo
                and self.distancia_entre_animales(animal) < 5  # Establece tu rango de reproducción
            ):
                # Intentar reproducirse
                nueva_cria = self.reproducir(animal)
                if nueva_cria:
                    return nueva_cria
        return None

    def distancia_entre_animales(self, otro_animal):
        x1, y1 = self.ubicacion
        x2, y2 = otro_animal.ubicacion
